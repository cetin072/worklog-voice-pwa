(() => {
  ...
  function institutionProvenance(row) {
    return text(
      row?.metadata?.fieldProvenance?.institution
      || row?.metadata?.institution_provenance
      || "",
    );
  }

  function trustedInstitution(row) {
    const provenance = institutionProvenance(row);
    return ["user_selected", "user_confirmed"].includes(provenance)
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .search-result[aria-busy="true"]{opacity:.72}
  ...
  function createPanel() {
    ...
    if (!searchScreen) return null;

    let panel = document.getElementById("worklogSearchDetail");
    if (!panel) {
      ...
      panel.setAttribute("aria-label", "검색 기록 상세");
      panel.innerHTML = `
    ...
    const close = panel.querySelector("#worklogSearchDetailClose");
    const status = panel.querySelector("#worklogSearchDetailStatus");
    const card = panel.querySelector("#worklogSearchDetailCard");
    if (!close || !status || !card) return null;
    return { panel, close, status, card };
  }

  function ensureElements() {
    if (elementsCache?.panel?.isConnected) return elementsCache;
    ensureStyles();
    elementsCache = createPanel();
    if (elementsCache) wire(elementsCache);
    return elementsCache;
  }

  async function readError(response) {
    const body = await response.json().catch(() => ({}));
    return text(body?.message || body?.msg || body?.error_description || body?.hint || body?.details)
      || `기록을 불러오지 못했습니다. (${response.status})`;
  }
  ...
  async function fetchRecord(recordId, signal) {
    ...
    if (!config?.supabaseUrl || !config?.publishableKey) {
      throw new Error("기록 조회 설정을 불러오지 못했습니다.");
    }
    ...
    if (!row || uuid(row.id) !== recordId) {
      throw new Error("이 기록을 찾지 못했거나 볼 권한이 없습니다.");
    }
    return row;
  }
  ...
  function clearDetailHistoryFlag() {
    if (!window.history?.replaceState || !window.history.state?.[DETAIL_HISTORY_KEY]) return;
    const next = { ...(window.history.state || {}) };
    delete next[DETAIL_HISTORY_KEY];
  ...
  function wire(els) {
    ...
    window.addEventListener("popstate", (event) => {
      const detailId = uuid(event.state?.[DETAIL_HISTORY_KEY]);
      if (detailId) {
    ...
    window.addEventListener("worklog:platform-auth-changed", () => {
      clearDetailHistoryFlag();
      if (!els.panel.hidden) hideDetail(els, { returnFocus: false });
    });
  }

  async function open(recordId, trigger) {
    const normalizedId = uuid(recordId);
    if (!normalizedId) throw new Error("열 수 없는 기록입니다.");
    const els = ensureElements();
    if (!els) throw new Error("기록 상세 화면을 준비하지 못했습니다.");
    await showDetail(els, normalizedId, trigger);
  }
  ...
  window.WorklogSearchDetail = Object.freeze({ open, close });
})();