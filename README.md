# 업무기록 Voice PWA

갤럭시에서 홈 화면 아이콘을 눌러 업무를 말하고, 기존 Notion `🎙 업무 통합 기록` DB에 저장하는 소형 PWA입니다.

## 현재 안정판

- 기준: v1.1 / 2026-08-25
- Production: `https://worklog-voice-pwa.netlify.app`
- 배포: Netlify
- 원본 저장소: 이 GitHub 저장소
- Notion 토큰 및 앱 접근키는 GitHub에 저장하지 않고 Netlify 환경변수로만 관리

## 구조

`Android PWA → Web Speech API / Android 키보드 음성입력 → Netlify Function → Notion API`

브리핑 후보 기능은 별도 Netlify Function이 Notion 미완료 업무를 읽어 규칙 기반 우선순위를 계산하며, 기존 저장 흐름과 분리되어 있습니다.

## 현재 기능

- 큰 `말하기` 버튼
- 한국어 음성 인식
- 사용자가 중지할 때까지 계속 듣기 및 무음 종료 시 자동 재시작
- 녹음 중 Screen Wake Lock 지원 기기에서 화면 자동 꺼짐 방지
- 인식 결과 확인 및 수정
- 작성 중 내용 자동 임시저장 및 앱 재실행 시 복구
- 기관 / 상태 / 유형의 가벼운 규칙 기반 자동선택
- 태장 관련 일부 음성 오인식은 기관 분류만 보정하고 음성원문은 그대로 보존
- 금액 / 담당자 / 기한 / 후속조치 선택 입력
- 저장 직전 마지막 음성 결과 반영 대기
- 저장 성공 진동 및 완료 표시
- 기존 Notion `🎙 업무 통합 기록` DB에 저장
- AI API 사용 없음
- PWA 홈 화면 설치 지원

## v1.2 브리핑 후보

- 앱 첫 화면에 `오늘 브리핑` 카드 표시
- Notion의 완료되지 않은 업무를 읽어 오늘 우선순위 TOP 3~5 계산
- 오늘 기한/일정, 기한 경과, 확인 필요, 후속조치, 진행/대기 상태를 규칙 기반으로 가중
- 오늘 확정 일정과 14일 안의 다가오는 일정 표시
- 사용자가 직접 `새로고침` 가능
- Notion 저장 성공 직후 브리핑 자동 갱신
- 별도 AI API나 추가 유료 서비스 없음

## 환경변수

Netlify에서만 관리합니다.

- `NOTION_TOKEN`
- `APP_ACCESS_KEY`
- `NOTION_DATA_SOURCE_ID`

비밀값을 코드, Issue, PR, README에 넣지 않습니다.

## 운영 원칙

현재 `main`은 실제 운영 가능한 안정판입니다.

향후 패치는 다음 흐름을 기본으로 합니다.

1. GitHub Issue로 변경 목적 기록
2. 기능 브랜치 생성
3. 코드 수정
4. 테스트
5. Draft PR 검수
6. 승인 후 `main` 반영
7. Netlify production 배포 확인

사용자의 명시적 승인 없이 큰 구조 변경, Notion DB 파괴적 변경, 토큰 노출, 운영 데이터 삭제를 하지 않습니다.

## 모바일 검수 시나리오

1. 앱을 열고 오늘 브리핑이 표시되는지 확인한다.
2. `말하기`를 누르고 한 문장을 말한다.
3. 10초 이상 침묵한다.
4. 다시 말했을 때 기존 문장 뒤에 이어지는지 확인한다.
5. 말이 끝나자마자 저장해도 마지막 단어가 빠지지 않는지 확인한다.
6. 작성 중 앱을 닫았다 다시 열었을 때 임시 기록이 복구되는지 확인한다.
7. `Notion에 저장` 후 진동/완료 표시가 나오고 실제 Notion 원장에 저장되는지 확인한다.
8. 저장 성공 직후 오늘 브리핑이 다시 조회되는지 확인한다.

## 향후 후보 기능

- 금액 / 담당자 / 기한 추출 정확도 개선
- 네트워크 장애 시 전송 대기열
- 사용자별 로그인 / 개인 데이터 분리
- 푸시 알림
- 이메일 / 카카오톡 등 외부 전달 채널
- 최소한의 AI 정리 기능은 비용과 안정성 확인 후 별도 검토
