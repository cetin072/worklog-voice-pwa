...
import { searchMyWorkRecords, type WorkRecordSearchResult } from '@/src/platform/worklog-api';
...
export function WorkRecordSearch({ client }: { client: PlatformSupabaseClient }) {
  ...
  return <View style={styles.root}>
    ...
    {items.map((item) => <View key={item.workRecordId} style={styles.result}><Text style={styles.title}>{item.title}</Text><Text style={styles.meta}>{statusLabel(item.status)}{item.institution ? ` · ${item.institution}` : ''}{recordedAtLabel(item.recordedAt) ? ` · ${recordedAtLabel(item.recordedAt)}` : ''}</Text>{item.snippet ? <Text style={styles.snippet}>{item.snippet}</Text> : null}</View>)}
...
const styles = StyleSheet.create({
  ...
  result: { gap: 4, borderTopWidth: 1, borderTopColor: '#eceef1', paddingTop: 13 },
