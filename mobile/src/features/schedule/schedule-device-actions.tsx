...
import { CHECKING_DEVICE_SYNC, collectDeviceSyncResult, type DeviceSyncResult } from './device-sync-result';
...
export function ScheduleDeviceActions({ schedule, compactOnly = false }: { schedule: BriefingSchedule; compactOnly?: boolean }) {
  ...
  const [syncResult, setSyncResult] = useState<DeviceSyncResult>(CHECKING_DEVICE_SYNC);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const result = await collectDeviceSyncResult({
        reminders: () => synchronizeScheduleReminders({ scheduleId, title: deviceSchedule.title, scheduleStartsAt: startsAt }),
        calendar: () => synchronizeScheduleToPreferredCalendar(deviceSchedule),
      });
      const [preferred, mapping, savedReminders] = await Promise.allSettled([getPreferredCalendarId(), getScheduleCalendarMapping(scheduleId), listScheduleReminders(scheduleId)]);
      if (version !== requestVersion.current) return;
      if (mapping.status === 'fulfilled') {
        setCalendarMapping(mapping.value);
        setCalendarId(mapping.value?.calendarId || (preferred.status === 'fulfilled' ? preferred.value : null));
      } else {
        result.calendar = { status: 'error', message: '캘린더 연결 기록을 읽지 못했습니다. 반영 완료로 표시하지 않습니다.' };
      }
      if (preferred.status === 'rejected') result.calendar = { status: 'error', message: '기본 캘린더 설정을 확인하지 못했습니다.' };
      if (savedReminders.status === 'fulfilled') setReminders(savedReminders.value);
  ...
  async function loadCalendars() {
    ...
    try {
      ...
      // Opening a list does not select an account or grant sync consent.
  ...
  function chooseCalendar(calendar: WritableCalendarOption) {
    setCalendarId(calendar.id);
    setMessage(`${calendar.ownerAccount || calendar.title} · 이 일정의 전송 대상으로 선택했습니다. 기본 캘린더는 바뀌지 않습니다.`);
  }
  ...
  async function toggleReminder(offsetMinutes: ReminderOffsetMinutes) {
    ...
    try {
      ...
      const verified = await synchronizeScheduleReminders({ scheduleId, title: schedule.title || '업무수첩 일정', scheduleStartsAt: startsAt });
  ...
  const connectedCalendarLabel = calendarSynced && calendarMapping
    ...
    : syncResult.calendar.status === 'checking' ? '캘린더 확인 중' : syncResult.calendar.status === 'error' ? '⚠ 캘린더 반영 확인 필요' : '미연결';
  ...
  return <View style={styles.root}>
    ...
    {syncResult.calendar.message && syncResult.calendar.status !== 'checking' ? <Text style={styles.warning}>{syncResult.calendar.message}</Text> : null}
    {syncResult.reminders.message && syncResult.reminders.status === 'error' ? <Text style={styles.warning}>{syncResult.reminders.message}</Text> : null}
    {syncResult.reminders.status === 'error' || syncResult.calendar.status === 'error' || syncResult.calendar.status === 'permission' ? <Button title="기기 반영 다시 확인" disabled={busy} onPress={() => setRetryKey((v) => v + 1)} /> : null}
    {!compactOnly && expanded ? <View style={styles.expanded}>
      <View style={styles.section}>
        ...
        {calendarMapping ? <Button title="캘린더에서 이 일정 제거" disabled={busy} onPress={() => void removeCalendarEvent()} /> : null}
      </View>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.presetGrid}>{REMINDER_PRESETS.map((preset) => {
          ...
          return <Pressable key={preset.offsetMinutes} accessibilityRole="button" disabled={busy || (!available && !active)} style={[styles.preset, active ? styles.presetActive : null, !available ? styles.presetDisabled : null]} onPress={() => void toggleReminder(preset.offsetMinutes)}>
    ...
    {message ? <Text style={syncResult.reminders.status === 'error' || syncResult.calendar.status === 'error' ? styles.warning : styles.message}>{message}</Text> : null}
  </View>;
}
