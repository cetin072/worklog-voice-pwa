...
export default function HomeScreen() {
  ...
  const [editActionKind, setEditActionKind] = useState<WorklogActionKind | null>(null);
  const [editInitialActionKind, setEditInitialActionKind] = useState<WorklogActionKind | null>(null);
  const [editCanConvertAction, setEditCanConvertAction] = useState(false);
  ...
  async function loadTaskEditorDetails(pageId: string, fallbackTitle = '', fallbackDate = '') {
    ...
    try {
      ...
      setEditActionKind(details.actionKind || null);
      setEditInitialActionKind(details.actionKind || null);
      setEditCanConvertAction(details.canConvertAction === true);
      setEditReady(true);
    } catch (nextError) {
  ...
  async function openTaskEditor(task: BriefingTask) {
    ...
    setEditTime('');
    setEditActionKind(null);
    setEditInitialActionKind(null);
    setEditCanConvertAction(false);
    setEditReady(false);
    setEditStatus('');
    setEditStatusTone('neutral');
  ...
  function updateVisibleRecordTitle(recordId: string, nextTitle: string) {
    setBriefing((current) => {
  ...
  async function saveTaskEditor() {
    ...
    const actionKindChanged = Boolean(editActionKind && editActionKind !== editInitialActionKind);
    setEditBusy(true);
    setEditStatus('');
    setEditStatusTone('neutral');
    try {
      const result = await updateWorklogDetails(session.access_token, {
        ...
        ...(actionKindChanged && editActionKind ? { actionKind: editActionKind } : {}),
      });
      const visibleTitle = result.title?.trim() || nextTitle;
      updateVisibleRecordTitle(recordId, visibleTitle);
      setEditBusy(false);
      setEditReady(false);
      setEditStatus(result.actionChanged
  ...
  <WorkRecordEditSheet
    ...
    actionKind={editActionKind || undefined}
    canConvertAction={editCanConvertAction}
