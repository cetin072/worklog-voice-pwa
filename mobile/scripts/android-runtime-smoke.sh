...
# QA-only native Korean STT gate. This branch is never merged.
adb shell am force-stop "$PACKAGE" || true
adb logcat -c || true
adb shell am start -W -a android.intent.action.VIEW -d 'worklog://prehuman-stt' -p "$PACKAGE"

STT_XML=/tmp/worklog-prehuman-stt.xml
stt_done=0
for attempt in $(seq 1 150); do
  adb shell uiautomator dump /sdcard/worklog-prehuman-stt.xml >/dev/null 2>&1 || true
  adb pull /sdcard/worklog-prehuman-stt.xml "$STT_XML" >/dev/null 2>&1 || true

  if grep -q 'PREHUMAN STT PASS' "$STT_XML" 2>/dev/null; then
    stt_done=1
    break
  fi
  if grep -q 'PREHUMAN STT FAIL' "$STT_XML" 2>/dev/null; then
    echo "Native Korean STT FAIL"
    cat "$STT_XML" || true
  ...
  if grep -q "Quickstep isn't responding" "$STT_XML" 2>/dev/null; then
...
echo "Native Korean STT PASS"
grep -o 'text="[^"]*"' "$STT_XML" | tail -20 || true
adb logcat -d -t 1000 | grep -E 'PREHUMAN_STT_PASS|ReactNativeJS|quick-voice' | tail -200 || true
