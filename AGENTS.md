...
## 사용자 프리뷰 요청 전 QA Gate — 필수

- 사용자를 기본 버그 탐지용 디버거로 사용하지 않는다.
- 사용자에게 `프리뷰를 열어보세요`, `눌러보세요`, `캡처해서 보내주세요`라고 요청하기 전에 반드시 `docs/PREVIEW_QA_GATE.md`를 읽고 해당 검수 항목을 실제로 수행한다.
- `Netlify Ready`, `npm test 통과`, `코드상 문제없어 보임` 중 하나만으로 사용자 검수를 요청하지 않는다.
- 브라우저/자동화/정적 검사로 잡을 수 있는 오류는 사용자에게 넘기기 전에 먼저 제거한다.
- 새 기능에는 가능한 범위에서 전용 자동 테스트와 대표 정상 경로 검증을 추가한다.
- Worker/WASM/모델/동적 import/외부·로컬 런타임 자산을 쓰는 기능은 실제 배포 산출물과 접근 경로까지 확인한다.
- 사용자가 실기에서 발견한 버그는 단순 수정으로 끝내지 말고, 같은 유형을 다음부터 사전에 잡을 테스트나 검증 규칙을 추가한다.
- 사용자 실기 검수는 실제 기기에서만 판단 가능한 카메라 동작, 터치 조작감, 실제 문서 OCR 정확도, 모바일 가독성 같은 항목에 한정한다.
- 위 Gate를 통과하지 못했거나 확인하지 않은 항목이 있으면 사용자에게 프리뷰 검수를 요청하지 말고 먼저 내부 검수를 계속한다.
