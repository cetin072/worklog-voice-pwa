...
begin
  ...
  if char_length(v_request_id) < 16 or char_length(v_request_id) > 100 then raise exception 'CLIENT_REQUEST_ID_INVALID'; end if;
  ...
  insert into public.work_records (
    workspace_id, created_by_user_id, client_request_id, title, content, original_text,
    record_type, status, institution, amount, follow_up, recorded_at, due_at, metadata
  ) values (
    v_workspace_id, v_user_id, v_request_id, p_title, coalesce(p_content, ''), coalesce(p_original_text, ''),
    p_record_type, p_status, nullif(btrim(coalesce(p_institution, '')), ''), p_amount,
    nullif(btrim(coalesce(p_follow_up, '')), ''), p_recorded_at, p_due_at, coalesce(p_metadata, '{}'::jsonb)
  )
  on conflict on constraint work_records_workspace_client_request_id_key
  do update set
    title = excluded.title, content = excluded.content, original_text = excluded.original_text,
    record_type = excluded.record_type, status = excluded.status, institution = excluded.institution,
    amount = excluded.amount, follow_up = excluded.follow_up, recorded_at = excluded.recorded_at,
    due_at = excluded.due_at, metadata = excluded.metadata
  returning id into v_work_record_id;

  insert into public.source_refs (
    workspace_id, created_by_user_id, client_request_id, entity_type, entity_id,
    source_type, source_id, source_excerpt, metadata
  ) values (
    v_workspace_id, v_user_id, v_request_id, 'work_record', v_work_record_id::text,
    'direct', v_request_id, p_source_excerpt, jsonb_build_object('source', 'quick_worklog')
  )
  on conflict on constraint source_refs_workspace_client_request_id_key
  do update set
    entity_type = excluded.entity_type, entity_id = excluded.entity_id, source_type = excluded.source_type,
    source_id = excluded.source_id, source_excerpt = excluded.source_excerpt, metadata = excluded.metadata
  ...
  if p_schedule_starts_at is not null then
    ...
    if v_schedule_id is null then
      insert into public.schedules (
        ...
        jsonb_build_object('source', 'quick_worklog', 'sourceRequestId', v_request_id,
      ...
      set title = btrim(p_schedule_title), description = coalesce(p_original_text, ''), starts_at = p_schedule_starts_at,
          ...
          metadata = coalesce(s.metadata, '{}'::jsonb) || jsonb_build_object(
            'source', 'quick_worklog', 'sourceRequestId', v_request_id,
            'workRecordId', v_work_record_id, 'sourceRefId', v_source_ref_id)
