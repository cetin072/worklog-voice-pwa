-- Issue #255 follow-up: suppress split-record false positives from shared original_text.
-- Direct raw/normalized/alias matches keep precedence. original_text-only matches are
-- retained as a fallback, but only once per shared source text when no sibling has
-- a stronger match. SECURITY INVOKER keeps the existing RLS boundary.
