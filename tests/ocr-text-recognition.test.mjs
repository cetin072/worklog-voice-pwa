import test from "node:test";
import assert from "node:assert/strict";
import path from "node:path";
import {createWorker} from "tesseract.js";

// Upstream Tesseract.js v7 simple PNG fixture, used in the same data-URL form as upstream tests.
const SIMPLE_PNG="data:image/png;base64,";

test("local OCR extracts the upstream expected text",{timeout:90000},async()=>{
  const worker=await createWorker("eng",1,{
    langPath:path.resolve("public/vendor/tessdata"),
    cacheMethod:"none",
    gzip:true,
    logger:()=>{}
  });
  try{
    const {data}=await worker.recognize(SIMPLE_PNG);
    assert.equal(String(data?.text||""),"Tesseract.js\n");
  }finally{
    await worker.terminate();
  }
});
