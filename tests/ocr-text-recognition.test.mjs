import test from "node:test";
import assert from "node:assert/strict";
import path from "node:path";
import {createWorker} from "tesseract.js";

// Upstream Tesseract.js simple PNG fixture. The source fixture represents the word Tesseract.js.
const SIMPLE_PNG=Buffer.from("","base64");

test("local OCR actually extracts text from a privacy-safe fixture",{timeout:90000},async()=>{
  const worker=await createWorker("eng",1,{
    langPath:path.resolve("public/vendor/tessdata"),
    cacheMethod:"none",
    gzip:true,
    logger:()=>{}
  });
  try{
    const {data}=await worker.recognize(SIMPLE_PNG);
    const text=String(data?.text||"").trim();
    assert.ok(text.length>0,"OCR returned no text from upstream fixture");
    assert.match(text,/Tess/i,"OCR did not extract the expected fixture word");
  }finally{
    await worker.terminate();
  }
});
