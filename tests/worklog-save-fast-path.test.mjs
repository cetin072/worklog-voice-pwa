...
test("clear timed meeting intent is saved as a confirmed Schedule in the same RPC", async () => {
  ...
  const result = await createWorklogDataCoreAdapter({ client }).persistFast(meeting);
  assert.equal(calls.length, 1);
...
test("save_my_worklog_with_schedule RPC keeps RLS active and authenticated-only", () => {
...
test("schedule save is linked and idempotent by the same worklog request id", () => {
  ...
  assert.match(migration, /insert into public\.schedules/i);
  assert.match(migration, /metadata ->> 'sourceRequestId' = v_request_id/i);
  assert.match(migration, /'workRecordId', v_work_record_id/i);
  assert.match(migration, /'sourceRefId', v_source_ref_id/i);
