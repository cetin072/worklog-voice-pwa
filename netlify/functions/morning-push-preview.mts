...
export default async (req:Request, _context:Context) => {
  ...
  if(!accessToken) return json(401,{error:"LOGIN_REQUIRED",message:"로그인 후 오늘 브리핑 알림을 테스트할 수 있습니다."});
  ...
  try{
    ...
    const payload=buildMorningPushPayload(preview);
    if(!payload){
      return json(200,{ok:true,delivered:false,empty:true,message:"오늘 보낼 브리핑 업무나 일정이 없습니다."});
    }
    ...
    try{
      ...
      return json(error?.expired ? 410 : 502,{error:code,message:error?.expired ? "이 기기의 기존 알림 구독이 만료되었습니다. 다시 연결해주세요." : "오늘 브리핑 알림을 전송하지 못했습니다.",pushStatus:status || undefined});
    }
  }catch(error:any){
    const code=String(error?.code || "MORNING_PUSH_PREVIEW_FAILED");
    ...
    console.error("Morning Push preview failed",code,String(error?.message || "unknown").slice(0,160));
    return json(400,{error:code,message:String(error?.message || "오늘 브리핑 알림 테스트에 실패했습니다.")});
  }
};

export const config:Config={path:"/api/morning-push-preview",method:["POST"]};
